...
biz.query = {
	...
	terminalPressBiz : function(){
	    
	    var rows = biz.table.getSelections();
	    if (rows.length < 1) {
            bsAlert('提示',"请选择要催办的工单");
            return;
        }
        var ids = new Array();
...
biz.query.form = {
	...
	loadSystem : function() {
		$.ajax({
			...
			success : function(data) {
				biz.query.form.systemList = data
			}
		});
		return biz.query.form.systemList;
	},
	init : function() {
		var queryFormList = [ {
			name : "bizId",
